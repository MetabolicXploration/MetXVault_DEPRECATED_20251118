...
Downloaded from https://www.pnas.org by 190.6.73.186 on January 20, 2025 from IP address 190.6.73.186.
...
$$\begin{aligned} \min & \sum_{i=1}^{m} y_i, \\ \text{s.t. } & S \cdot \nu = 0, \,\,\nu_{\text{min}} \le \nu \le \nu_{\text{max}}, \\ & \nu_j = 0, j \in A, \\ & \text{for } 1 \le i \le m \\ & \nu_i - y_i(\nu_{\text{max},i} - \boldsymbol{w}_i^u) \le \boldsymbol{w}_i^u, \\ & \nu_i - y_i(\nu_{\text{min},i} - \boldsymbol{w}_i^l) \ge \boldsymbol{w}_i^l, \\ & \boldsymbol{y}_i \in \{0, 1\}, \end{aligned} \tag{1}$$

$$\begin{aligned} \boldsymbol{\nu}_{i}^{\mu} &= \boldsymbol{\nu}_{i} + \boldsymbol{\delta}|\boldsymbol{\nu}_{i}| + \boldsymbol{\varepsilon}, \\ \boldsymbol{\nu}_{i}^{l} &= \boldsymbol{\nu}_{i} - \boldsymbol{\delta}|\boldsymbol{\nu}_{l}| - \boldsymbol{\varepsilon}, \end{aligned}$$
